...
export function ContextualTargeting() {
  return (
    <Section
      ...
      className="bg-context-bg pb-[97px]"
    >
      ...
      <ul className="mt-12 grid gap-x-3.5 gap-y-[15px] md:grid-cols-3">
        {cases.map((item) => (
          <li
            key={item.title}
            className="flex gap-3.5 rounded-2xl border border-line bg-card px-[22px] pt-[22px] pb-[18.5px]"
          >
            <span
              aria-hidden="true"
            ...
            />
            <div>
              <h3 className="text-[13px] leading-tight font-bold text-fg">
                {item.title}
              </h3>
