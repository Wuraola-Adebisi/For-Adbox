...
export function HeroCarrier() {
  ...
  const [initialTheme] = useState(theme) // captured once; later changes go through postMessage
  ...
  useEffect(() => {
    iframeRef.current?.contentWindow?.postMessage({ type: 'theme', theme }, window.location.origin)
  }, [theme])
  ...
  return (
    ...
    >
      <iframe
        ref={iframeRef}
        // The iframe is a separate document and can't see our data-theme attribute or
        // prefers-color-scheme override on its own, so the theme at mount time is passed via the
        // query string; changes after that go through postMessage (see the effect above) rather
        // than reloading the src, so the scene doesn't reset or flash when toggling.
        src={`/carrier-360.html?theme=${initialTheme}`}
        title="AdBox carrier box, 3D view"
        aria-hidden="true"
        tabIndex={-1}
        scrolling="no"
        // allow-same-origin is required for postMessage theme sync below: without it the
        // iframe gets an opaque "null" origin, and both the parent's targetOrigin match and the
        // iframe's own event.origin check silently drop the message (confirmed by testing).
        // The file is our own static asset, not third-party content, so this is safe.
      ...
      <span ref={leftAnchorRef} aria-hidden="true" className="absolute top-[35%] left-[37%] size-px" />
      <span ref={rightAnchorRef} aria-hidden="true" className="absolute top-[30%] left-[63%] size-px" />
        ...
        {lines && (
          <svg className="pointer-events-none absolute inset-0 size-full overflow-visible">
            {[lines.a, lines.b].map((l, i) => (
              <g key={i}>
                <line x1={l.x1} y1={l.y1} x2={l.x2} y2={l.y2} stroke="#5b6b80" strokeWidth="1" opacity="0.55" />
                <circle cx={l.x2} cy={l.y2} r="2" fill="#5b6b80" opacity="0.7" />
              </g>
            ))}
          </svg>
        )}

        <div ref={chip1Ref} className={`${chip} top-[13.5%] left-[8%] w-max min-w-[150px] px-3 py-2.5`}>
          ...
          <div className="mt-1.5 flex items-baseline justify-between gap-2">
            <span className="text-[11px] font-bold whitespace-nowrap text-white">{ad.brand}</span>
            <span className="shrink-0 text-[8px] whitespace-nowrap text-[#8a96a8]">{ad.kind} · 5s spot</span>
          </div>
        </div>

        <div ref={chip2Ref} className={`${chip} top-[16.5%] right-[7%] w-[145px] px-3 py-2.5`}>
          <div className="text-[8px] text-[#8a96a8]">Views this hour</div>
          <div className="mt-0.5 flex items-baseline gap-3">
            <span className="text-base leading-5 font-bold text-white tabular-nums">{views.toLocaleString('en-US')}</span>
            <span className="text-[8px] font-semibold text-[#2ee6a6]">▲ 18%</span>
          </div>
          <div className="mt-1.5 flex items-center justify-between gap-2 text-[8px]">
            <span className="text-[#8a96a8]">Location</span>
            <span className="truncate font-semibold text-white">{ad.place}</span>
          </div>
        </div>

        <div className={`${chip} right-[8%] bottom-[10%] w-[134px] px-3 py-2.5`}>
